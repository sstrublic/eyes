...
// This is roughly the maximum reliable distance the sensor can measure, about ~3m.
#define MAX_DISTANCE            0xC00   // 3072mm

// Range of random numbers for blink/wink actions.
#define ACTION_RANGE            32767

// Minimum and maximum time between winks or blinks.
#define BLINK_TIMEOUT_MS        4000
#define BLINK_MAXIMUM_TIMEOUT_MS 8000
...
MOOD_STATE get_new_state(uint16_t distance, uint16_t distancelast, uint16_t setpoint)
{
    MOOD_STATE new_state;

    DEBUG_PRINT("%5d -> %5d: ", distancelast, distance);
...
bool warn_for_approach(uint16_t distance, uint16_t setpoint)
{
    ...
    if (distance < (uint16_t)((uint32_t)setpoint * 135 / 100)) {
        retval = true;
    }
    ...
    while(1)
    {
            ...
            if (0 == SW_GetValue()) {
                ...
                if (buttonheld > PROGRAM_CHECK_PERIODS) {
                    ...
                    if (distance < MAX_DISTANCE) {
                        DEBUG_PRINT("Storing distance setpoint as %d\r\n", distance);
        ...
        if (distance <= MAX_DISTANCE)
        {
            ...
            if ((now - actionlast) > ACTION_THRESHOLD_MS) {
                MOOD_STATE new_state = moodstate;\
                ...
                if (abs(distancelast - distance) > SENSOR_JITTER) {
                    new_state = get_new_state(distance, distancelast, setpoint);
                    if (new_state != moodstate) {
                        DEBUG_PRINT("%5u -> %5u: %s -> %s\r\n", distancelast, distance, statestr[moodstate], statestr[new_state]);
                    }
